...
RUN echo "source scl_source enable rh-ruby25; while true; do echo "Trying to start the web server..."; gunicorn --worker-tmp-dir /dev/shm -b 0.0.0.0:5000 -w 12 $FLASK_APP.wsgi:app --log-file /log/web_server; if [[ $? != 0 ]]; then echo "Failed to start the web server..."; sleep 1; else echo "Web server started! :D"; fi; done" > /scripts/start_gunicorn.sh
